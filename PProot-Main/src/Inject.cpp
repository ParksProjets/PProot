/*
 *
 * Inject: Functions for injecting code into winlogon.exe for disabling
 *         SAS (Alt+Ctrl+Del)
 *
 *
 * Pris de iTALC
...
DWORD GetPIDFromName( char *szProcessName )
{
    ...
    // Get Kernel32 handle
    if( !( hKernel = GetModuleHandle( L"Kernel32.dll" ) ) )
        return -1;
    ...
    while( bRes )
    {
        ...
        char* szAscii = Utils::ConvertLPWSTRToLPSTR(pe32.szExeFile);
...
static HWND WINAPI GetSASWnd( INJDATA *pData )
{
    ...
    return ( pData->fnFindWindow(
                Utils::ConvertLPCSTRToLPWSTR(pData->szClassName),
                Utils::ConvertLPCSTRToLPWSTR(pData->szWindowName)
            ) );
}
